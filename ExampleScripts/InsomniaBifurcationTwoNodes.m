...
%% Do the stuff...
  ...
  options.Integration.dt = 2^-14;
  options.Integration.iters = 2^12;
  
  options = SetDerivedParameters(options);
  options = SetInitialConditions(options);
  
  addpath(genpath('./Bifurcations'))
  options.Bifurcation.BifurcationParameter='nu_sn';
  options = SetBifurcationOptions(options);
  options.Bifurcation.BifurcationParameterIncrement =  0.005e2;
  options.Bifurcation.ErrorTolerance = 1.0e-6; 
  options.Bifurcation.MaxContinuations = 3;
